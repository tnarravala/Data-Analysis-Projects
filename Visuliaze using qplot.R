...
df.1960 <- mydata[mydata$Year == "1960",]
head(df.1960)
df.2013 <- mydata[mydata$Year != "1960",]
head(df.2013)

df.1960$Life.Exp <- Life_Expectancy_At_Birth_1960
df.2013$Life.Exp <- Life_Expectancy_At_Birth_2013

qplot(data = df.1960,x = Fertility.Rate, y = Life.Exp,color = Region)
qplot(data = df.2013,x = Fertility.Rate, y = Life.Exp,color = Region)
...
mymat<-matrix(c(3,5,8,4),nrow = ,ncol = 3,byrow = T)
mymat*mymat
